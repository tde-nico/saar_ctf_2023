...
int HttpGet_RedisCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
	...
	size_t path_len;
	const char *path = RedisModule_StringPtrLen(argv[1], &path_len);
	...
	if(path_len >= 8) {
		int ok = 0;
		if(strncmp(path, "/", 1) == 0) ok = 1;
		if(strncmp(path, "newest:", 1) == 0) ok = 1;
		if(strncmp(path, "country:", 1) == 0) ok = 1;
...
int NewUser_RedisCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
	...
	reply = RedisModule_Call(
			ctx, "ACL", "cscccccccccccccccccccccbbccccc", "SETUSER", argv[1],
			"on", "-@all", "+@read", "-@hash", "+SETNX", "+SADD", "+LPUSH", "+HSETNX", "+INCRBY", "-DUMP", "+@connection", "-COMMAND", "-RANDOMKEY", "+@pubsub",
